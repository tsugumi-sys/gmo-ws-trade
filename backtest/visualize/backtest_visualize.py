...
def cum_return_plot(ax: plt.Axes, cum_return: pd.Series) -> None:
    sns.lineplot(y=cum_return.to_numpy(), x=cum_return.index, ax=ax)
    ax.set_title("Cumulative Return")


def position_change_plot(ax: plt.Axes, position: pd.Series) -> None:
    sns.lineplot(y=position.to_numpy(), x=position.index, ax=ax)
    ax.set_title("Position Change")


def position_change_average_plot(ax: plt.Axes, position: pd.Series, rolling_period: int = 10):
